...
def home(request):
    transfer = list(Movie.objects.all())
    nexttransfer = NextMovies.objects.all()
    # return render(request, "home.html", {"transfer": transfer, "nexttransfer": nexttransfer})
    return render(request, "home.html", {"random_movies": transfer,"nexttransfer": nexttransfer})

def showprice(request):
    return render(request, "showprice.html")
...
def index(request):
    ...
    return render(request, "index.html", {"random_movies": random_movies})
...
def results(request, query):
    ...
    if query == 'all':
        ...
        if len(results) == 0:
            ...
            return render(request, "searchResults.html", {'movies': results})


def moviePage(request, movieName):
    return render(request, "moviePage.html", {"movie": Movie.objects.get(name=movieName)})
